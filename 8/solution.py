def resolve_entry(entry):
    known_segments_by_digit = {}

    # sort by length
    entry[0].sort(key=lambda e: len(e))

    # we already know these by their length
    known_segments_by_digit[1] = entry[0][0]
    known_segments_by_digit[7] = entry[0][1]
    known_segments_by_digit[4] = entry[0][2]
    known_segments_by_digit[8] = entry[0][9]

    # 0, 6 and 9 all have 6 segments, and two of them should contain the segments for one.
    # by process of elimination, we can determine which is digit 6 (should not contain both segments for 1)
    segments_6 = [entry[0][6], entry[0][7], entry[0][8]]
    x = ''.join(set(entry[0][6]).intersection(known_segments_by_digit[1]))
    y = ''.join(set(entry[0][7]).intersection(known_segments_by_digit[1]))
    z = ''.join(set(entry[0][8]).intersection(known_segments_by_digit[1]))
    if len(x) == 1:
        known_segments_by_digit[6] = segments_6.pop(0)
    elif len(y) == 1:
        known_segments_by_digit[6] = segments_6.pop(1)
    else:
        known_segments_by_digit[6] = segments_6.pop(2)

    # 2, 3 and 5 all have 5 segments.
    # similar to above, only 3 should contain all segments for 1
    segments_5 = [entry[0][3], entry[0][4], entry[0][5]]
    x = ''.join(set(entry[0][3]).intersection(known_segments_by_digit[1]))
    y = ''.join(set(entry[0][4]).intersection(known_segments_by_digit[1]))
    z = ''.join(set(entry[0][5]).intersection(known_segments_by_digit[1]))
    if len(x) == 2:
        known_segments_by_digit[3] = segments_5.pop(0)
    elif len(y) == 2:
        known_segments_by_digit[3] = segments_5.pop(1)
    else:
        known_segments_by_digit[3] = segments_5.pop(2)

    # now we need to separate 0 from 9, and 2 from 5

    # 9 should contain all of 4
    x = ''.join(set(segments_6[0]).intersection(known_segments_by_digit[4]))
    y = ''.join(set(segments_6[1]).intersection(known_segments_by_digit[4]))
    if len(x) == 4:
        known_segments_by_digit[0] = segments_6[1]
        known_segments_by_digit[9] = segments_6[0]
    else:
        known_segments_by_digit[0] = segments_6[0]
        known_segments_by_digit[9] = segments_6[1]

    # now we need 2 and 5

    # 6 should contain all of 5
    x = ''.join(set(segments_5[0]).intersection(known_segments_by_digit[6]))
    y = ''.join(set(segments_5[1]).intersection(known_segments_by_digit[6]))
    if len(x) == 5:
        known_segments_by_digit[2] = segments_5[1]
        known_segments_by_digit[5] = segments_5[0]
    else:
        known_segments_by_digit[2] = segments_5[0]
        known_segments_by_digit[5] = segments_5[1]

    # great! now what are the digits

    digits = []
    for output in entry[1]:
        for i, (k, v) in enumerate(known_segments_by_digit.items()):
            if set(output) == set(v):
                digits.append(str(k))
                break
...
def solve(puzzle_input):
    def parse_entry(entry):
        parts = [x.split(" ") for x in entry.split(" | ")]
        return (parts[0], parts[1])
    entries = [parse_entry(x) for x in puzzle_input.strip().split('\n')]

    count = 0
    for entry in entries:
        for output_digit in entry[1]:
